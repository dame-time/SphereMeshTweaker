...
Sphere deeperSphere(const Sphere& sphereA, const Sphere& sphereB, const glm::vec3& p)
{
    ...
    const BumperCapsuloid bc (-1, -1, sphereA, sphereB);
...
void SphereMesh::removeRedundantElements()
{
    ...
    for (auto& prysmoid : prysmoids)
    {
        ...
        if (uniquePrysmoids.find(key) != uniquePrysmoids.end())
            continue;
    ...
    for (auto &[index] : singletons)
    {
        ...
        if (uniqueSingletons.find(key) != uniqueSingletons.end())
            continue;
...
bool SphereMesh::loadFromFile(const char* path)
{
    ...
    if (header != "Sphere-mesh 2") {
        std::cerr << "Invalid file format header!" << std::endl;
        return false;
    }

    int nSpheres = 0, nPrysmoids = 0, nCapsuloids = 0;

    inputFile >> nSpheres >> nPrysmoids >> nCapsuloids;
    ...
    for (int i = 0; i < nPrysmoids; ++i) {
        ...
        inputFile >> index1 >> index2 >> index3;
    ...
    std::cout << "File loaded successfully from: " << path << std::endl;
...
bool SphereMesh::saveToFile(const char* path) const
{
    std::ostringstream fileContent;

    fileContent << "Sphere-mesh 2" << std::endl;
    fileContent << spheres.size() << " " << prysmoids.size() << " " << capsuloids.size() << std::endl;

    for (const auto& s : spheres)
        fileContent << s.center.x << " " << s.center.y << " " << s.center.z << " " << s.radius << std::endl;

    for (const auto& p : prysmoids)
        fileContent << p.indices[0] << " " << p.indices[1] << " " << p.indices[2] << std::endl;

    for (const auto& c : capsuloids)
    ...
    std::cout << "File saved successfully at: " << path << std::endl;
...
{
    std::cout << sm.bbox.maxCorner.x << " " << sm.bbox.maxCorner.y << " " << sm.bbox.maxCorner.z << std::endl;
    std::cout << sm.bbox.minCorner.x << " " << sm.bbox.minCorner.y << " " << sm.bbox.minCorner.z << std::endl;
    sphere = sm.spheres;
    bbox = sm.bbox;
...
bool BumperGrid::pushOutsidePrysmoid(glm::vec3 &p, glm::vec3 &n, const int bumperIndex, const char flags) const
{
    ...
    if (!(flags & IGNORE_OPP_SIDE_1))
        if (bp.m1.isBehind(p))
        {
            if (flags & DONT_FOLLOW_SIDE_1) return false;
    ...
    if (!(flags & IGNORE_OPP_SIDE_2))
        if (bp.m2.isBehind(p))
        {
            if (flags & DONT_FOLLOW_SIDE_2) return false;
    ...
    if (!(flags & IGNORE_OPP_SIDE_0))
        if (bp.m0.isBehind(p))
        {
            if (flags & DONT_FOLLOW_SIDE_0) return false;
...
int getRandomInVector(const std::vector<int> & v, const int randSeed){
    if (v.empty()) return -1;
...
bool BumperGrid::pushOutsideGrid (glm::vec3 &p, glm::vec3 &n) const
{
    ...
    const char flags = bumperAtPos & FLAGS_MASK;
    bumperAtPos >>= 4;

    if (bumper[bumperAtPos].shapeType == Bumper::SPHERE)
        return pushOutsideSphere(p, n, bumperAtPos);
